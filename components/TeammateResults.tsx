...
import { ProjectIdea, TeamMember, ScoredProfile, FilterOptions } from '../types';
...
interface TeammateResultsProps {
  project: ProjectIdea;
  suggestedTeammates: ScoredProfile[];
  onClear: () => void;
  currentUser: TeamMember;
  team: TeamMember[];
  onAddTeammate: (teammate: TeamMember | ScoredProfile) => void;
...
const TeammateResults: React.FC<TeammateResultsProps> = ({
  ...
  const [searchResults, setSearchResults] = useState<ScoredProfile[]>([]);
  const [isSearching, setIsSearching] = useState(false);
  const [filters, setFilters] = useState<FilterOptions>({});
  const [showSuccess, setShowSuccess] = useState(false);

  // Trigger confetti animation when team reaches 3+ members
  useEffect(() => {
    if (team.length >= 3 && !showSuccess) {
      setShowSuccess(true);
      // Auto-hide success message after 5 seconds
      setTimeout(() => setShowSuccess(false), 5000);
    }
  }, [team.length, showSuccess]);

  const handleSearch = () => {
    setIsSearching(true);
    const results = searchTeammatesByName(searchQuery, currentUser, filters);
  ...
  const handleFilterChange = (newFilters: FilterOptions) => {
    ...
    if (searchQuery.trim()) {
      setIsSearching(true);
      const results = searchTeammatesByName(searchQuery, currentUser, newFilters);
      setSearchResults(results);
      setIsSearching(false);
    }
  };
    ...
    <div className="animate-fade-in space-y-8">
      ...
      <div>
        ...
        <div className="bg-slate-700/50 p-4 rounded-lg border border-sky-500">
          ...
          <ProjectDetails idea={project} />
        </div>
      </div>

      {/* Section 2: Team Export */}
      {team.length > 0 && (
        <TeamExport project={project} team={team} />
      )}

      {/* Success Message */}
      {showSuccess && team.length >= 3 && (
        <div className="bg-green-500/20 border border-green-500 rounded-lg p-6 text-center animate-slide-in-up">
          <div className="text-4xl mb-2">🎉</div>
          <h3 className="text-2xl font-bold text-green-400 mb-2">Team Complete!</h3>
          <p className="text-slate-300">Your team is ready to start {project.title}</p>
        </div>
      )}
      ...
      {/* Section 4: Teammate Filters */}
      <TeammateFilters
        filters={filters}
        onFilterChange={handleFilterChange}
      />

      {/* Section 5: Teammate Search */}
      <div>
