...
const ProjectDetails: React.FC<ProjectDetailsProps> = ({ idea }) => {
  ...
  const handleShare = async () => {
    const shareText = `Check out this project: ${idea.title}\n\n${idea.description}${idea.scope ? `\n\nScope: ${idea.scope}` : ''}${idea.suggestedStack ? `\n\nStack: ${idea.suggestedStack}` : ''}`;
  ...
  return (
    <div className="border-t border-slate-600 mt-4 pt-4 animate-fade-in" style={{ animationDelay: '100ms' }}>
      ...
      <div className="mt-4">
        <button
          onClick={handleShare}
          className="w-full bg-emerald-600 text-white font-semibold py-2 px-4 rounded-md hover:bg-emerald-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-800 focus:ring-emerald-500 transition duration-300 flex items-center justify-center gap-2"
        >
